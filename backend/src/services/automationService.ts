    ...
    try {
      ...
      for (const employee of employees) {
        ...
        if (!attendance) {
          ...
          await emailService.sendAttendanceReminder(employee.id);
    ...
    try {
      ...
      for (const product of products) {
        ...
        if (totalStock <= product.lowStockThreshold) {
          ...
          await emailService.sendLowStockAlert(product.id);
  ...
  async checkOverdueInvoicesAndSendReminders() {
    try {
      ...
      for (const invoice of overdueInvoices) {
        ...
        await emailService.sendInvoiceReminder(invoice.id);
  ...
  async checkLateTasksAndSendReminders() {
    try {
      ...
      for (const task of lateTasks) {
        if (task.assignedTo) {
          // Send task reminder
          await emailService.sendTaskReminder(task.id);
  ...
  async generateMonthlyPayroll() {
    try {
      ...
      for (const employee of employees) {
        ...
        await emailService.sendPayrollNotification(employee.id, {
          baseSalary,
          overtime: overtimePay,
          bonuses,
          deductions,
