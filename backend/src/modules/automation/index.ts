...
router.get('/insights/cash-flow-forecast', async (req, res) => {
  try {
    ...
    const forecast = await insightsService.getCashFlowForecast(months);
    res.json(forecast);
  } catch (error) {
...
router.get('/insights/profitable-customers', async (req, res) => {
  try {
    ...
    const customers = await insightsService.getMostProfitableCustomers(limit);
    res.json(customers);
  } catch (error) {
...
router.get('/insights/kpi-analysis', async (req, res) => {
  try {
    const kpis = await insightsService.getKPIAnalysis();
    res.json(kpis);
  } catch (error) {
...
router.get('/insights/employee-performance', async (req, res) => {
  try {
    const performance = await insightsService.getEmployeePerformanceInsights();
...
router.get('/insights/inventory', async (req, res) => {
  try {
    const insights = await insightsService.getInventoryInsights();
    res.json(insights);
  } catch (error) {
...
router.post('/trigger/payroll', async (req, res) => {
  try {
    await automationService.triggerPayrollGeneration();
...
router.post('/trigger/recurring-invoices', async (req, res) => {
  try {
    await automationService.triggerRecurringInvoices();
...
router.post('/trigger/attendance-check', async (req, res) => {
  try {
    await automationService.triggerAttendanceCheck();
...
router.post('/trigger/low-stock-check', async (req, res) => {
  try {
    await automationService.triggerLowStockCheck();
