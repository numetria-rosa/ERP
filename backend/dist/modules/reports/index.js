...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const employeeCount = await prisma_1.default.employee.count();
        const customerCount = await prisma_1.default.customer.count();
        ...
        const recentEmployees = await prisma_1.default.employee.findMany({
            take: 5,
        ...
        const revenueByCustomer = await prisma_1.default.transaction.groupBy({
            by: ['invoiceId'],
            where: { type: 'income' },
            _sum: { amount: true },
        ...
        const customerRevenueData = await Promise.all(revenueByCustomer.map(async (item) => {
            if (item.invoiceId) {
